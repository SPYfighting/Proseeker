#!/usr/bin/env python3
"""
Hyperparameter Search (Standalone Version, B-GNN)
...
def split_by_unique_child_sequences(
    ...
    """
    Split dataset by unique child sequences to prevent data leakage.
    """
    ...
    print(f"Split Complete: Training samples={len(train_indices)}, Validation samples={len(val_indices)}")
    print(
...
def objective(trial, cfg: dict):
    ...
    dataset = PairDataset(cfg["training_pairs"], tokenizer, for_training=True, use_graph=use_gnn)
